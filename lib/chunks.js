...
(function(root, undefined) {
  ...
  //
  // The base function used in all of `Chunks`'s other functions.
  //
  ...
  Chunks.forEach = Chunks.each = function(obj, iterator, scope) {
    if (nativeEach && obj.nativeEach) {
      ...
      var i, keys = Chunks.keys(obj), length = keys.length;
      for (i = 0; i < length; i++) {
  ...
  // ### Chunks.sort
  //
  // Return a new array with the values from the `obj` sorted by numerical
  // or lexicographical order.
  //
  // - `obj` is either an array or an object.
  // - `by`  is a string.
  //
  Chunks.sort = function(obj, by) {
    var values  = [],
        results = null;
    Chunks.forEach(obj, function(val) {
      values.push(val);
    });
    if (by === 'numerical' || by === 'num') {
